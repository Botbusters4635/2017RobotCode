...
*      Author: abiel
*
*      Source code from "https://github.com/KHEngineering/SmoothPathPlanner",
*      It was modified for it to be implemented on our robot.
*/
#include "EctoPath.h"

EctoPath::EctoPath(path InPath, double motorReduction) {
...
string EctoPath::ParseToString(singlePath InPath) {
...
double EctoPath::RadiansToDegrees(double radians) {
	return (radians * 180) / M_PI;
}

double EctoPath::deegrestoRadian(double deegres) {
	return (deegres * M_PI) / 180;
	
}

path EctoPath::ResizePath(path inPath, unsigned int x, unsigned int y) {
...
//
path EctoPath::InitializePath(path in) {
	in.resize(1);
...
path EctoPath::Inject(path orig, int numToInject) {
	path morePoints;
	path testPoints;
...
path EctoPath::smoother(path pathIn, double weight_data, double weight_smooth,
                        double tolerance) {
	...
	while (change >= tolerance) {
		...
		for (unsigned int i = 1; i < pathIn.size() - 1; i++) {
			for (unsigned int j = 0; j < pathIn[i].size(); j++) {
				...
				newPath[i][j] += weight_data * (pathIn[i][j] - newPath[i][j])
				                 + weight_smooth
				                   * (newPath[i - 1][j] + newPath[i + 1][j]
				                      - (2.0 * newPath[i][j]));
...
path EctoPath::NodeOnlyWayPoints(path pathIn) {
	path li;
	
	li.push_back(pathIn[0]);
	
	for (unsigned int i = 1; i < pathIn.size() - 1; i++) {
		double vector1 = atan2((pathIn[i][1] - pathIn[i - 1][1]),
		                       pathIn[i][0] - pathIn[i - 1][0]);
		double vector2 = atan2((pathIn[i + 1][1] - pathIn[i][1]),
		                       pathIn[i + 1][0] - pathIn[i][0]);
...
path EctoPath::velocity(path smoothPath, double timeStep) {
...
path EctoPath::velocityFix(path smoothVelocity, path origVelocity,
                           double tolerance) {
	...
	while (abs(difference[difference.size() - 1]) > tolerance) {
		...
		for (unsigned int i = 1; i < fixVel.size() - 1; i++) {
...
singlePath EctoPath::errorSum(path origVelocity, path smoothVelocity) {
	...
	for (unsigned int i = 1; i < origVelocity.size(); i++) {
		...
		tempSmoothDist[i] = smoothVelocity[i][1] * timeStep
		                    + tempSmoothDist[i - 1];
...
vector<int> EctoPath::injectionCounter2Steps(double numNodeOnlyPoints,
...
//					numFinalPoints = pointsTotal;
		...
		for (int i = 1; i <= 5; i++)
			for (int j = 1; j <= 8; j++)
				for (int k = 1; k < 8; k++) {
					pointsFirst = i * (numNodeOnlyPoints - 1)
					              + numNodeOnlyPoints;
...
void EctoPath::leftRight(path smoothPath, double robotTrackWidth) {
	unsigned int smoothPathSize = smoothPath.size();
	...
	for (unsigned int i = 0; i < smoothPathSize - 1; i++)
		gradient[i][1] = atan2(smoothPath[i + 1][1] - smoothPath[i][1],
	...
	for (unsigned int i = 0; i < gradientSize; i++) {
		leftPath2[i][0] = (robotTrackWidth / 2 * cos(gradient[i][1] + M_PI / 2))
		                  + smoothPath[i][0];
		leftPath2[i][1] = (robotTrackWidth / 2 * sin(gradient[i][1] + M_PI / 2))
		                  + smoothPath[i][1];
		
		rightPath2[i][0] = robotTrackWidth / 2 * cos(gradient[i][1] - M_PI / 2)
		                   + smoothPath[i][0];
		rightPath2[i][1] = robotTrackWidth / 2 * sin(gradient[i][1] - M_PI / 2)
		                   + smoothPath[i][1];
...
singlePath EctoPath::getXVector(path arr) {
	singlePath temp;
...
singlePath EctoPath::getYVector(path arr) {
	singlePath temp;
	temp.resize(arr.size());
...
path EctoPath::transposeVector(path pathIn) {
	path temp;
...
void EctoPath::setPathAlpha(double alpha) {
	pathAlpha = alpha;
}

void EctoPath::setPathBeta(double beta) {
	pathAlpha = beta;
}

void EctoPath::setPathTolerance(double tolerance) {
	pathAlpha = tolerance;
}

void EctoPath::printPath(path inPath) {
	for (unsigned int i = 0; i < inPath.size(); i++) {
...
void EctoPath::calculate(double totalTime, double timeStep,
                         double robotTrackWidth) {
	cout << " STARTED " << endl;
	this->timeStep = timeStep;
	nodeOnlyPath = NodeOnlyWayPoints(origPath);
	
	vector<int> inject = injectionCounter2Steps(nodeOnlyPath.size(), totalTime,
	                                            timeStep);
	...
	for (unsigned int i = 0; i < inject.size(); i++) {
		if (i == 0) {
			...
			smoothPath = smoother(smoothPath, pathAlpha, pathBeta,
			                      pathTolerance);
	...
	smoothCenterVelocity = smoother(smoothCenterVelocity, velocityAlpha,
	                                velocityBeta, velocityTolerance);
	smoothLeftVelocity = smoother(smoothLeftVelocity, velocityAlpha,
	                              velocityBeta, velocityTolerance);
	smoothRightVelocity = smoother(smoothRightVelocity, velocityAlpha,
	                               velocityBeta, velocityTolerance);
